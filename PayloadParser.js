function parseUplink(device, payload)
{
    ...
    if(parsed.nivelSkid != null) {
        var skid = device.endpoints.byAddress("1");

        if (skid != null)
            skid.updateGenericSensorStatus(parsed.nivelSkid);
    }
    
    if(parsed.temperaturaSkid != null) {
        var temp = device.endpoints.byAddress("2");

        if (temp != null)
            temp.updateTemperatureSensorStatus(parsed.temperaturaSkid);
    }

    if(parsed.vBateriaNodo != null) {
        var bat = device.endpoints.byAddress("3");

        if (bat != null)
            bat.updateGenericSensorStatus(parsed.vBateriaNodo);
    }
    
    if(parsed.flagsNodo != null) {
        var flags = device.endpoints.byAddress("4");

        if (flags != null)
            flags.updateGenericSensorStatus(parsed.flagsNodo);
    }
